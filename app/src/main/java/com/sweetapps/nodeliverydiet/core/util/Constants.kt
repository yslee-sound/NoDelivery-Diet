...
package com.sweetapps.nodeliverydiet.core.util
...
object Constants {
    ...
    // 전역 매핑 유틸: 사용자 설정 라벨을 수치로 변환
    fun costPerOrder(label: String): Int = when (label) {
        "저" -> 15000
        "중" -> 35000
        "고" -> 50000
        else -> 35000
    ...
    fun frequencyPerWeek(label: String): Double = when (label) {
        "주 1회 이하" -> 1.0
        "주 2~3회" -> 2.5
        "주 4회 이상" -> 5.0
        else -> 2.5
    ...
    fun mealHours(label: String): Int = when (label) {
        "짧음" -> 2
        "보통" -> 4
        "김" -> 6
        else -> 4
